...
rule FASTP:
	...
	#fastqc -t {threads} {input.R1} {input.R2} -o {wildcards.time}/s01_fastqc/ 2>&1 > {log} 
        ...
        fastp --in1 {input.R1} --in2 {input.R2} --adapter_sequence CTGTCTCTTATACACATCT --adapter_sequence_r2 AGATGTGTATAAGAGACAG --out1 {output[0]} --out2 {output[1]} --detect_adapter_for_pe --trim_poly_g --trim_poly_x --length_required 50 --json {wildcards.time}/s01_fastqc/{wildcards.sample}.fastp.Clean.json --html {TIME}/s01_fastqc/{wildcards.sample}.fastp.Clean.html --thread {threads} -p 2>&1  >> {log}
...
rule BWA:
	...
	bwa mem -M -Y -t {threads} -R '@RG\\tID:{wildcards.sample}\\tSM:{wildcards.sample}\\tLB:{wildcards.sample}\\tPL:{params.platform}' {REFERENCE} {input.R1} {input.R2} | samtools view -Sbh -F 256 - -o {wildcards.time}/s02_alignment/s020_bwa/{wildcards.sample}.bam 2>{log}
        ...
        samtools sort -m 1G -@ {threads} -T {wildcards.time}/s02_alignment/s020_bwa/tmp_{wildcards.sample} -o {output.bam} {TIME}/s02_alignment/s020_bwa/{wildcards.sample}.bam 2>>{log}
	...
	samtools index -@ {threads} {output.bam} 2>>{log} 
        ...
        {GATK} --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=4" ValidateSamFile -I {output.bam} -MODE SUMMARY -O {wildcards.time}/s02_alignment/s020_bwa/{wildcards.sample}.sort.bam.Val.txt
	#######################
	"""
...
rule MarkDup:
	...
	#######################
        ...
        {GATK} --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=4  -Djava.io.tmpdir={wildcards.time}/s02_alignment/s021_Dedup/tmp_{wildcards.sample}" MarkDuplicates -AS true -M {output.metrics} -I {input.bam} -O {output.bam} --REMOVE_DUPLICATES false --VALIDATION_STRINGENCY SILENT  --CREATE_INDEX true 2>&1 >{log}
        mv {wildcards.time}/s02_alignment/s021_Dedup/{wildcards.sample}.dd.bai {output.index}
        {GATK} --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=4" ValidateSamFile -I {output.bam} -MODE SUMMARY -O {wildcards.time}/s02_alignment/s021_Dedup/{wildcards.sample}.dd.bam.Val.txt
	#######################
	"""
...
rule GATK_BQSR:
	...
	#######################
        ...
        # bam validation
        {GATK} --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=4" ValidateSamFile -I {output.bam} -MODE SUMMARY -O {wildcards.sample}.marked.BQSR.bam.Val.txt
