...
rule all:
    input:
        ...
        expand("{time}/s03_variant/GATK/{sample}.g.vcf.gz", sample=SAMPLES, time=TIME),
...
rule GATK_BQSR:
    input:
        bam=temp("{time}/s02_alignment/s021_Dedup/{sample}.dd.bam"),
        bai=temp("{time}/s02_alignment/s021_Dedup/{sample}.dd.bam.bai")
    output:
...
rule GenotypeGVCFs:
    ...
    output:
        temp("{time}/s03_variant/GATK/{family}_HC_calls_combined.vcf.gz")
    conda:
        "Genomics"
    log:
...
rule VariantRecalibrator:
    ...
    output:
        snprecal=temp("{time}/s03_variant/GATK/{family}_SNP.recal"),
        snptranches=temp("{time}/s03_variant/GATK/{family}_SNP.tranches"),
        snpplot="{time}/s03_variant/GATK/{family}_SNP.plots.R",
        indelrecal=temp("{time}/s03_variant/GATK/{family}_INDEL.recal"),
        indeltranches=temp("{time}/s03_variant/GATK/{family}_INDEL.tranches"),
        indelplot="{time}/s03_variant/GATK/{family}_INDEL.plots.R",
        snpvqsr=temp("{time}/s03_variant/GATK/{family}_merged_VQSR_SNP.vcf.gz"),
