#########################################
# SNP calling using GATK HC for Tumor pair samples
# by Xingmin Liu
...
SAMPLES={"ZJQ-LICT", "ZJQ-N"}
FAMILY={"ZJQ"}
TARGET="/SlurmDatabase/Clinical/2023/0721_CRC/hg38_lifted_agilent_region.V6.bed"
...
rule all:
    input:
        ...
        expand("{time}/s03_variant/GATK/{sample}_VQSR_SNP_INDEL.vcf.gz", sample=SAMPLES, time=TIME)
...
rule GATK_BQSR:
	...
	#######################
        ...
        {GATK} SplitIntervals -R {REFERENCE} -L {TARGET} --scatter-count 30 -O {wildcards.time}/s02_alignment/s022_Brecal/interval-files
        {GATK} SplitIntervals -R {REFERENCE} -L {REFBED} --scatter-count 30 -O {wildcards.time}/s02_alignment/s022_Brecal/interval-files_Apply

        echo "Working on GATK BQSR"
        for sam in {wildcards.sample}
        do 
            for i in `seq -f '%04g' 0 29`
            do
            ...
            do 
                ...
                {GATK} --java-options "-Xmx4G -XX:+UseParallelGC -XX:ParallelGCThreads=4" \
                ApplyBQSR -L {wildcards.time}/s02_alignment/s022_Brecal/interval-files_Apply/${{i}}-scattered.interval_list \
                -R {REFERENCE} -bqsr ${{bqfile}} \
                --static-quantized-quals 10 --static-quantized-quals 20 --static-quantized-quals 30 \
                -I {input.bam} -O ${{outputf}} 2>&1 >> {log} &
            done
            wait
        done
        ...
        rm -f {wildcards.time}/s02_alignment/s022_Brecal/{wildcards.sample}.marked.BQSR.bai
...
rule VariantRecalibrator:
    input:
        "{time}/s03_variant/GATK/{sample}_HC_calls.vcf.gz"
    output:
        snprecal="{time}/s03_variant/GATK/{sample}_SNP.recal",
        snptranches="{time}/s03_variant/GATK/{sample}_SNP.tranches",
        snpplot="{time}/s03_variant/GATK/{sample}_SNP.plots.R",
        indelrecal="{time}/s03_variant/GATK/{sample}_INDEL.recal",
        indeltranches="{time}/s03_variant/GATK/{sample}_INDEL.tranches",
        indelplot="{time}/s03_variant/GATK/{sample}_INDEL.plots.R",
        snpvqsr="{time}/s03_variant/GATK/{sample}_VQSR_SNP.vcf.gz",
        indelvqsr="{time}/s03_variant/GATK/{sample}_VQSR_SNP_INDEL.vcf.gz"
    conda:
        "Genomics"
    log:
...
rule Mutect2:
    input:
        bam="{time}/s02_alignment/s022_Brecal/{sample}.marked.BQSR.bam"
    output:
        "{time}/s03_variant/GATK/{sample}_mutect2_somatic_oncefiltered.vcf.gz"
    threads: 4
    resources:
        mem_mb=8000
    conda:
        "Genomics"
    log:
        "{time}/s03_variant/{sample}_Mutect2.log"
    shell:"""
    #######################
    #!/bin/bash
    echo "Working on Mutect2"
    """
