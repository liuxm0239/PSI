...
REFNPZ="/HD101TB/bioinfo/liuxingmin/Clinical/2023/0810_nipt/2023-08-15/s02_alignment/s022_Brecal/reference_output.npz"
...
rule all:
    input:
        ...
        expand("{time}/s03_variant/WisecondorX/{sample}.npz", sample=SAMPLES, time=TIME)
...
rule WisecondorX:
    input:
        bam="{time}/s02_alignment/s022_Brecal/{sample}.marked.BQSR.bam"
    output:
        npz="{time}/s03_variant/WisecondorX/{sample}.npz"
    threads: 2
    resources: mem_mb=4000
    conda: "nipt"
    log:
        "{time}/s03_variant/{sample}_nipt.log"
    shell: """
	#######################
        #!/bin/bash
        echo "Working on WisecondorX"
	WisecondorX convert {input.bam} {output.npz}
        WisecondorX predict {output.npz} {REFNPZ} {time}/s03_variant/WisecondorX/{wildcards.sample}  --plot --bed
	#######################
	"""
############
