...
SAMPLES={"RB1", "RB2"}
REFERENCE="/HD101TB/bioinfo/reference/hg38/hg38.fasta"
...
rule BWA:
	...
	bwa mem -M -Y -t {threads} -R '@RG\\tID:{wildcards.sample}\\tSM:{wildcards.sample}\\tLB:{wildcards.sample}\\tPL:{params.platform}' {REFERENCE} {input.R1} {input.R2} | samtools view - -b -S -F 256 -t {REFERENCE}.fai -o {output.raw_bam}

        echo "Working on samtools sort"
        samtools sort -m 3G -@ {threads} -T {wildcards.time}/s02_alignment/s020_bwa/tmp_{wildcards.sample} -o {output.bam} {output.raw_bam} 2>{log}
